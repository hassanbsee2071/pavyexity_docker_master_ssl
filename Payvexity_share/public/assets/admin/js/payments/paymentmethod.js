...
$(document).ready(function() {
  ...
  $("#payment_method").on("change", function(e) {
    ...
    } else if ($("#payment_method").val() == 'virtual_card') {
      var html = $("#credit_card").html();
      $("#payment_type_fields").html(html);
    }
    else {
...
function changePaymentMethod(value) {
  ...
  if (value == "bank_account") {
    ...
    if (flag == 0) {
      ...
      jQuery(function($) {
        ...
        $("[data-payment='cc-number']").payment('formatCardNumber');
        $("[data-payment='cc-exp']").payment('formatCardExpiry');
        $("[data-payment='cc-cvc']").payment('formatCardCVC');
        ...
        $('#payment-form').submit(function(e) {
          if(!$.payment.validateCardExpiry($("[data-payment='cc-exp']").payment('cardExpiryVal')))
          e.preventDefault();
          if(!$.payment.validateCardNumber($("[data-payment='cc-number']").val())) {
            e.preventDefault();
          }
  
          var cardType = $.payment.cardType($('.cc-number').val());
          console.log();
          $("[data-payment='cc-number']").toggleInputError(!$.payment.validateCardNumber($("[data-payment='cc-number']").val()));
          $("[data-payment='cc-exp']").toggleInputError(!$.payment.validateCardExpiry($("[data-payment='cc-exp']").payment('cardExpiryVal')));
          $("[data-payment='cc-cvc']").toggleInputError(!$.payment.validateCardCVC($("[data-payment='cc-cvc']").val(), cardType));
  ...
  $('#payment-form input').on('keyup', function() {
    ...
    $('#payment-form input').each(function() {
      ...
      if($("payment_method").val() == 'credit_card' && $("[data-payment='cc-number']") !== null && $("[data-payment='cc-number']").val().length < 19) {
        $('.SubmitButton').attr('disabled', 'disabled');
        empty = true;
        return false;
      }
