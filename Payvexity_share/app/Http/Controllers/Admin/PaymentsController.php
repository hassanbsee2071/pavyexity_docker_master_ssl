...
$new_company = 0;
    ...
    $new_company = CompanySetting::where('company_name',$companyid)->first();
// dd($new_company->id);
     
    }else{
        $new_company = CompanySetting::find($companyid);
    }
    
    $data['companyid']  = $new_company->id;

    $data['is_bank_allowed']  = $new_company->is_bank_allowed;
    $data['is_credit_allowed']  = $new_company->is_credit_allowed;
}else{
    ...
    $data['companyid']  = $companySetting->id;
    
    $data['is_bank_allowed']  = $companySetting->is_bank_allowed;
    $data['is_credit_allowed']  = $companySetting->is_credit_allowed;
...
// dd($companySetting);
    ...
    // dd($new_company->is_credit_allowed);
    Config::set('meta.title_suffix', '| '.$arr[4]);
    if($arr[0]!='global') {
        $data['if_amount'] = 1; 
   ...
   public function deletePaymentLink($id)
   {
    //    dd('sdsddssd');
       try {
        ...
        return redirect()->route('online.payment.links')->withFlashError('Payment Link Not Deleted!');
